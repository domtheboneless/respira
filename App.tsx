...
const App: React.FC = () => {
  ...
  const [view, setView] = useState<'landing' | 'onboarding' | 'auth' | 'dashboard'>('landing');
          ...
          try {
            ...
            console.error("Profile load error", e);
             if (e.code === '42501' || e.message?.includes('403')) {
                setConfigError("Database permission error. Please run the SQL setup commands.");
             }
          }
        }
      } catch (e: any) {
  ...
  const handleOnboardingComplete = async (stats: UserStats) => {
    ...
    if (userId) {
      try {
        ...
        if (e.code === '42501' || e.message?.includes('403')) {
          setConfigError("Database permission denied. Please ensure you have run the RLS policies in Supabase SQL Editor.");
        }
      }
    } else {
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-6 text-center animate-fadeIn">
        ...
        {configError && (
          <div className="bg-red-50 text-red-600 p-4 rounded-xl mb-6 max-w-md">
            ...
            <p className="text-sm mt-1">Row Level Security (RLS) is enabled but policies might be missing.</p>
          </div>
        )}

        {!configError && (
           <p className="text-gray-600 max-w-md mb-8 leading-relaxed">
            To enable cloud sync and persistence, please set the <code className="bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded font-mono text-sm border border-amber-100">SUPABASE_URL</code> and <code className="bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded font-mono text-sm border border-amber-100">SUPABASE_ANON_KEY</code> environment variables.
          </p>
        )}
        ...
        <div className="bg-white p-8 rounded-3xl shadow-2xl border border-gray-100 text-left w-full max-w-lg">
          ...
          <p className="text-sm text-gray-500 mb-4">Run this in your Supabase SQL Editor to fix permissions:</p>
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <header className="bg-white border-b border-gray-100 py-4 px-6 flex justify-between items-center sticky top-0 z-40 backdrop-blur-md bg-white/80">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xl font-extrabold text-gray-800 tracking-tight">Respira</span>
        </div>
        
        {view === 'landing' && (
          <LanguageSelector current={language} onSelect={handleLanguageSelect} />
        )}
        ...
        {userId && (
          ...
          >
            <span className="text-xs font-bold uppercase tracking-wider">{TRANSLATIONS[language].logout}</span>
        ...
        {view === 'landing' && (
          <div className="px-4 text-center py-20 animate-fadeIn">
            <h1 className="text-5xl font-extrabold text-gray-900 mb-4">
              {language === 'en' ? "Take your life back" : "Riprenditi la tua vita"}
            </h1>
      ...
      <footer className="py-6 text-center text-gray-400 text-sm">
        Respira &copy; {new Date().getFullYear()} - {language === 'en' ? "Breathe better, live more." : "Respira meglio, vivi di più."}
      </footer>
    </div>
  );
};
