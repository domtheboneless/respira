...
import toast from "react-hot-toast"; // <- aggiungi questo import in cima al file
...
const Auth: React.FC<Props> = ({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			if (isLogin) {
				...
				if (user) {
					...
					if (stats) {
						...
						toast.error(t.auth_error_no_profile);
			...
			console.error(err);
			// Solo se non abbiamo già mostrato un toast specifico sopra
			if (!err.message?.includes("Invalid login credentials")) {
				toast.error(err.message || t.auth_error_not_found);
			}
