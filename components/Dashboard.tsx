...
const Dashboard: React.FC<Props> = ({ stats, language, onUpdateStats }) => {
  ...
  const [cravingStep, setCravingStep] = useState<'rating' | 'low' | 'high'>('rating');
  ...
  const handleUrgeAnalysis = async () => {
    setIsLoadingAi(true);
    // Fetch AI phrase regardless of path, customized slightly
    const timeStr = `${timeDiff.d}${t.days} ${timeDiff.h}${t.hours} ${timeDiff.m}${t.minutes}`;
    const phrase = await getMotivationalPhrase(language, stats.dreamGoal, timeStr);
    setAiPhrase(phrase);
  ...
  const updateAngle = (clientX: number, clientY: number) => {
    ...
    // Map 0-360 to 1-10, clamping to bottom gap if needed
    // We want a partial circle, say 270 degrees total (active area)
    // -135deg to +135deg visually.
    
    // Simplified: Use full circle for logic but clamp value
    let val = Math.round((angle / 360) * 10) + 1;
    // Fix alignment so top is 5ish? No, let's just map 0-360 directly to 1-10 linearly
    // 0 deg = 1, 360 deg = 11 (wrap).
  ...
  // Color calculation for slider
  const getRatingColor = (r: number) => {
    if (r < 4) return '#22c55e'; // Green
    if (r < 7) return '#eab308'; // Yellow
    return '#ef4444'; // Red
  };
    ...
    <div className="max-w-4xl mx-auto p-4 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center justify-center">
          <span className="text-gray-500 font-medium mb-1">{t.money_saved}</span>
          <span className="text-4xl font-bold text-green-600">{stats.currency}{moneySaved.toFixed(2)}</span>
        </div>
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center justify-center">
          <span className="text-gray-500 font-medium mb-1">{t.cigs_not_smoked}</span>
          <span className="text-4xl font-bold text-indigo-600">{cigsSaved}</span>
        </div>
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center">
          <span className="text-gray-500 font-medium mb-1">{t.time_passed}</span>
      ...
      <div className="bg-gradient-to-br from-indigo-600 to-purple-700 p-8 rounded-3xl text-white shadow-lg relative overflow-hidden">
        <div className="relative z-10">
          <h3 className="text-lg font-medium opacity-80 mb-2">{t.dream_progress}</h3>
          ...
          <div className="flex justify-between text-sm">
            ...
            <span>{stats.currency}{stats.dreamCost}</span>
          </div>
        </div>
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 h-80">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData}>
            ...
            />
            <Line type="monotone" dataKey="saved" stroke="#4f46e5" strokeWidth={3} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      {showCravingModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-3xl max-w-md w-full p-8 relative shadow-2xl overflow-hidden flex flex-col min-h-[400px]">
            <button 
            ...
            {cravingStep === 'rating' && (
              <div className="flex flex-col items-center justify-center flex-1 animate-fadeIn">
                <h2 className="text-2xl font-bold text-gray-800 mb-6">{t.urge_modal_title}</h2>
                <div className="relative w-64 h-64 mb-6">
                  ...
                  >
                    ...
                    {/* Active Arc (approximate based on urgeRating) */}
                    <circle 
                      cx="100" cy="100" r="80" fill="none" 
                      stroke={getRatingColor(urgeRating)} 
                      strokeWidth="20"
                      strokeDasharray={`${(urgeRating / 10) * 502} 502`} // 2*PI*80 = ~502
                      strokeLinecap="round"
                      transform="rotate(-90 100 100)"
                      className="transition-all duration-100 ease-out"
                    />
                    {/* Center Text */}
                    <text x="100" y="115" textAnchor="middle" fontSize="60" fontWeight="bold" fill={getRatingColor(urgeRating)}>
                      {urgeRating}
                    </text>
                  </svg>
                  <p className="text-center text-gray-400 text-sm mt-2">{t.urge_rating_label}</p>
                </div>
                <button 
            ...
            {cravingStep === 'low' && (
              <div className="flex flex-col items-center justify-center flex-1 animate-fadeIn text-center">
                ...
                <h3 className="text-2xl font-bold text-gray-800 mb-2">{t.urge_low_title}</h3>
                <p className="text-gray-600 mb-6">{t.urge_low_text}</p>
                
                {/* Countdown Timer */}
                <div className="text-5xl font-mono font-bold text-indigo-600 mb-6 tracking-widest">
                  {Math.floor(countdown / 60).toString().padStart(2, '0')}:{(countdown % 60).toString().padStart(2, '0')}
                </div>

                {!isLoadingAi && aiPhrase && (
                  <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100 mb-6 text-sm">
                    <p className="text-indigo-900 italic">"{aiPhrase}"</p>
                  </div>
                )}

                <button 
                  onClick={() => setShowCravingModal(false)}
                  className="px-8 py-3 bg-white border-2 border-indigo-100 text-indigo-600 rounded-xl font-bold hover:bg-indigo-50"
                >
                  {t.urge_btn_resisted}
                </button>
              </div>
            ...
            {cravingStep === 'high' && (
              <div className="flex flex-col items-center justify-center flex-1 animate-fadeIn text-center">
                ...
                <h3 className="text-2xl font-bold text-gray-800 mb-2">{t.urge_high_title}</h3>
                <p className="text-gray-600 mb-6">
                  {t.urge_high_text
                    .replace('{cost}', `${stats.currency}${costPerCig.toFixed(2)}`)
                    .replace('{dream}', stats.dreamGoal)
                  }
                </p>

                <div className="w-full bg-red-50 p-4 rounded-xl border border-red-100 mb-6">
                  <p className="text-red-700 text-sm font-medium">-{stats.currency}{costPerCig.toFixed(2)} {language === 'en' ? "lost from savings" : "persi dai risparmi"}</p>
                </div>
      ...
      {/* Legacy Lapse Modal (for direct button click) */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-3xl max-w-md w-full p-8 relative shadow-2xl">
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-800 mb-4">{t.lapse_modal_title}</h2>
